...
// var redirect_url = BaseURL + 'sociallogin/' + itemId + '/' + shopId;
// console.log(appID);return false;
// newwindow = window.open('https://www.facebook.com/dialog/share?app_id=' + appID + '&display=popup&href=' + encry_urlss + '&redirect_uri=' + encry_urlss, 'name', 'height=600,width=600');
...
// });
	$(document).on('click', '#reportflag', function () {
		$("#reportflag").attr("disabled", "disabled");
		...
		if (loguserid == 0) {
			...
			$.ajax({
				...
				success: function (responce) {
					$("#reportflag").removeAttr("disabled");
					var data = '<i class="fa fa-flag-o"></i><span class="trn" data-toggle="modal" data-target="#undoreport_order">Undo reporting</span>';
					//$('.reportitem').html(data);
					$('#reportmodal').attr('data-target','#undoreport_order');
					$("#report_text").html('UnReport');
					$("#report_order").modal('hide');
	...
	$(document).on('click', '#unreportflag', function () {
		$("#unreportflag").attr("disabled", "disabled");
		var baseurl = getBaseURL();
		var loguserid = $("#loguserid").val();
		var translator = $('body').translate({
		t: dict
		...
		if (loguserid == 0) {
			...
			$.ajax({
				...
				success: function (responce) {
					$("#unreportflag").removeAttr("disabled");
					var data = '<i class="fa fa-flag-o"></i><span class="trn" data-toggle="modal" data-target="#report_order">Report Inappropriate</span>';
					//$('.reportitem').html(data);
					$("#report_text").html('Report');
					$('#reportmodal').attr('data-target','#report_order');
					$("#undoreport_order").modal('hide');
					translator.lang(sessionlang);
				}
			});
		}
...
if (keyCode != 8 && keyCode != 9 && keyCode != 13 && keyCode != 37 && keyCode != 38 && keyCode != 39 && keyCode != 40 && keyCode != 46 && keyCode != 110 && keyCode != 190) {
	...
	$('#list_createid').click(function () {
		...
		if ($.trim(newlist_val) != "" && typeof (newlist_val != 'undefined')) {
			...
			function (datas) {
				datas = $.trim(datas);
				datas = datas.split('-_-');
				if (datas[1] != '' && typeof (datas[1] != 'undefined')) {
					if (datas[1] == 'exists') {
						$("#existslisterr").show();
						setTimeout(function () {
							$('#existslisterr').fadeOut('slow');
						}, 5000);
						return false;
					} else {
...
//alert(eleid);
if (id != '') {
	$.post(baseurl + 'deletefeedcomments', {
		"addid": id,
		"logid": logid
	},
	function (datas) {
...
return false;
	...
	$("#foll" + usrid).hide();
	$("#unfoll" + usrid).hide();
	var cmtval = '<span id="unfoll' + usrid + '"><div class="btn folow_btn_user_profile editProf"><a href="javascript:void(0);" onclick="deletefollows(' + usrid + ')" class="trn">Unfollow</a></div></span>';
...
return false;
	...
	$("#foll" + usrid).hide();
	$("#unfoll" + usrid).hide();
	var cmtval = '<span id="foll' + usrid + '"><div class="btn folow_btn_user_profile editProf"><a href="javascript:void(0);" onclick="getfollows(' + usrid + ')" class="trn">Follow</a></div></span>';
	$("#changebtn" + usrid).html(cmtval);
	var sessionlang = $("#languagecode").val();
	var translator = $('body').translate({
		t: dict
...
return false;
	...
	$("#folls" + shopid).hide();
	$("#unfolls" + shopid).hide();
	var cmtval = '<span id="unfolls' + shopid + '"><div class="btn user_unfollowers"><a href="javascript:void(0);" onclick="deleteshopfollows(' + shopid + ')" class="trn">Unfollow Store</a></div></span>';

	$("#changebtnn" + shopid).html(cmtval);
	var sessionlang = $("#languagecode").val();
	var translator = $('body').translate({
		t: dict
	});
...
return false;
	...
	$("#folls" + shopid).hide();
	$("#unfolls" + shopid).hide();
	var cmtval = '<span id="folls' + shopid + '"><div class="user_followers_butn btn"><a href="javascript:void(0);" onclick="getshopfollows(' + shopid + ')" class="trn">Follow Store</a></div></span>';
	$("#changebtnn" + shopid).html(cmtval);
	var sessionlang = $("#languagecode").val();
	var translator = $('body').translate({
		t: dict
...
if (process == 'Delivered') {
	$('#status' + oid).html("Delivered");
	var add_content = '<div class="col-lg-2 col-md-2 col-sm-2 col-xs-6 text-center no-hor-padding responsive-none"><a class="btn border_left_grey_shop font-color oder-option" href="javascript:void(0);" onclick="dispute('+oid+');">Create Disputes</a></div>';
	$('#statusbutton' + oid).append(add_content);
...
function markprocess_details(oid, process) {
	var BaseURL = getBaseURL();
	var status = ".status" + oid;
	var curlist = "." + process + oid;
	var markloader = '';
...
$.ajax({
	...
	data: {
		'postid': postid,
		"hashtags": oldtags['status' + postid]
	},
	beforeSend: function () {
//$('.postcommentloader').show();
$(feedselector).css({
...
function removestatusimg(val) {
	...
	$('#image_computer').val('');
	$('#show_url').attr({
		src: baseurl + 'media/avatars/thumb150/usrimg.jpg'
	});
	$('#statusimg-cont').hide();
}
/*********************Post status start*******************************/
...
function validggusersave() {
	...
	'&size=' + size + '&qty=' + qty + '&zipcode=' + zipcode + '&recipient=' + recipient;
	$.ajax({
		url: baseurl,
		type: "GET",
		dataType: "text",
		success: function (responce) {
			if(responce == '/2')
			{
					$('#recipient_name_err').hide();
					$('#recipient_check_name_err').hide();
					$('#recipient_check_recipient_err').show();
					$("#comment_msgemail").keydown(function () {
						$('#recipient_name_err').hide();
						$('#recipient_check_name_err').hide();
						$('#recipient_check_recipient_err').hide();
					});
					return false;
			}
			if (responce == 'failed') {
				$("#recipient_check_name_err").show();
				return false;
			} else {
...
var meta_image = $("#fullimgtag").val();
var appID = $("#default_fb_appid").val();
var desc = $("#default_site_desc").val();
var title = $.trim($('.gift_heading').html());
$('#meta_title').attr('content', title);
$('#meta_image').attr('content', meta_image);
$('#meta_description').attr('content', desc);
...
$('.facebook').attr('href', 'https://www.facebook.com/dialog/share?app_id=' + appID + '&display=popup&href=' + encry_urlss);
...
//alert(responce);
$('#noticnt').hide();
window.location.href = BaseURL + "push_notifications";
...
function show_writequestion() {
	...
	if (loguserid == 0) {
		...
		$("#write-question-ans-modal").modal('show');
	}
}

function read_other_answers(question_id = null, question = null) {

	var BaseURL = getBaseURL();
	let loguserid = $('#loguserid').val();
	//var que = atob(question);
	//$('.readansquestion').html(que);

		//alert(BaseURL);
		$.ajax({
			type: "POST",
			url: BaseURL + 'getanswers',
			data: {
			...
			success: function (datas) {
				var splitdata = datas.split('ans#list');
				var getauthor = splitdata[1].split('author_id');

				$('#question_id').val(question_id);
				if(getauthor[1] == loguserid)
				{
					$('#answer_forms').hide();
				}
				$('#answer-list-question').html(getauthor[0]);
				$('.readansquestion').html(splitdata[0]);
		},
		dataType: 'html'
		});
		$("#read_answers").modal('show');
	//}
}
...
function itemcou(id) {
	...
	var likebtncnt = $("#likebtncnt").val();
	var liketxt = $(".like-txt" + id).html();
//console.log(likebtncnt+"==="+liketxt);
	/* latest fixes 4.1 */
	var loguserid = $("#loguserid").val();
	//alert("loguserid="+loguserid+"likebtncnt"+likebtncnt);
	if (loguserid == 0) {
		window.location.href = BaseURL + "login";
		return false;
	}
	var imgselectorattr = $("#img_id" + id).val();
	$("#selectimgs").attr("src", imgselectorattr);
	//console.log(liketxt+' '+likebtncnt);
	if (loguserid == 0) {
		window.location.href = BaseURL + "login";
		return false;
	} else if (liketxt == likebtncnt) {
		...
		function (datas) {
			...
			//console.log("like");
			//alert("liked");
			$('#like-icon' + id).attr("src", BaseURL + "img/Heart filled@2x.png");
			$('.like-icon' + id).attr("src", BaseURL + "img/Heart filled@2x.png");
			$('#like-icon' + id).attr("alt", 'liked');
			//alert("liked img");
			$(".like-txt" + id).html(likedbtncnt);
			$("#like-txt" + id).html(likedbtncnt);
			return false;
		...
		function (datas) {
			$(".like-counter").html(datas);
			//$("input[type=checkbox]").removeAttr('checked');
			var url=BaseURL + "img/like.png";
			//alert("unlike="+url);
			//console.log("unlike"+id+"=="+url);
			$('#like-icon' + id).attr("src", url);
			$('.like-icon' + id).attr("src", url);
			$('#like-icon' + id).attr("alt", 'unliked');
			//alert("unliked img");
			$("#like-txt" + id).html(likebtncnt);
			$(".like-txt" + id).html(likebtncnt);
			return false;
...
function itemcouview(id) {
	//alert("itemcouview"+id);
	$("#listerr").hide();
	var addusrlist = true;
	unfantid = id;
	var BaseURL = getBaseURL();
...
//console.log(likebtncnt+"==="+liketxt);
	...
	if (loguserid == 0) {
		...
		function (datas) {
			...
			$(".like-counter").html(datas[0]);
			//console.log("like");
			//alert(BaseURL + "images/icons/Heart_after_like@2x.png");
			//console.log("like view"+id+"=="+url);
			$('#like-icon' + id).attr("src", BaseURL + "images/icons/Heart_after_like@2x.png");
			$('.like-icon' + id).attr("src", BaseURL + "images/icons/Heart_after_like@2x.png");
		...
		function (datas) {
			$(".like-counter").html(datas);
			//$("input[type=checkbox]").removeAttr('checked');
			var url=BaseURL + "images/icons/like.png";
			//alert("unlike view"+id+"=="+url);
			//alert(BaseURL + "images/icons/like.png");
			//console.log("unlike view"+id+"=="+url);
			$('#like-icon' + id).attr("src", BaseURL + "images/icons/like.png");
			$('.like-icon' + id).attr("src", BaseURL + "images/icons/like.png");
			$('#like-icon' + id).attr("alt", 'unliked');
			//alert("unliked img");
			$("#like-txt" + id).html(likebtncnt);
...
$('.like-icon' + id).attr("src", BaseURL + "images/icons/cate_heart.png");
$('#like-icon' + id).attr("src", BaseURL + "images/icons/cate_heart.png");
...
$('.like-icon' + id).attr("src", BaseURL + "images/icons/Heart.png");
$('#like-icon' + id).attr("src", BaseURL + "images/icons/Heart.png");
$("#like-txt" + id).html(likebtncnt);
return false;
}
...
function review(sellerid, userid) {
	...
	if (orderid == "" || orderid == null || orderid == '0') {
		...
		$("#raterr").show();
		$("#raterr").html("Please select order id");
		translator.lang(sessionlang);
		setTimeout(function () {
			$('#raterr').fadeOut('slow');
		}, 5000);
	} else if (rating_val == "" || rating_val == 0) {
		$("#raterr").removeAttr('data-trn-key');
		$("#raterr").show();
		...
		$.ajax({
			...
			data: {
				...
				"seller_id": sellerid,
				"rate_val": rating_val,
				"review_tit": review_title,
				"review_cont": review_content,
...
function questionsubmit()
{
	...
	if(question == '')
	{
		$('#errorspan_question').text('Please enter your questions here').css('color','red');
		$('#errorspan_question').fadeOut(3000);
		return false;
	}

	$('#commentssave').prop('disabled', true);
	$("#write-question-ans-modal").modal('hide');
	$("#postaquestion").modal('hide');

	questions = $.trim(question);
	itemid = $('#item_id').val();
	userid = $('#loguserid').val();
	usernames = $('#usernames').val();
	userimges = $('#userimges').val();
	baseurl = getBaseURL();
	logid = $("#loguser_id").val();
	questions = strip(questions);

	//alert( userid + itemid + question );

	
	$.ajax({
		type: "POST",
		url: baseurl + 'senddata',
		data: {
			"user_id": userid,
			"item_id": itemid,
			"content": question,
		...
		success: function (datas) {
				if(datas != 'false')
				{
					//let obj = jQuery.parseJSON( datas );
					//let html = '<div class="sold-by-prof-pic-cnt col-xs-2 col-lg-1 padding-right0"><a href="https://prodev.hitasoft.in/fantacy5.0/fantacy/people/'+usernames+'" class="url"><div class="sold-by-prof-pic" style="background-image:url(http://localhost/projects/fantacy-4.0/media/avatars/thumb70/usrimg.jpg);background-repeat:no-repeat;"></div></a></div><div class="comment-row col-xs-12 col-sm-12 comment delecmt_2"><a href="http://localhost/projects/fantacy-4.0/people/'+usernames+'" class="url"><div class="bold-font txt-uppercase comment-name">'+usernames+'</div></a><div class="bold-font">'+obj.content+'</div></div>'; 
					//$('#questions-answers').append(html);
					window.location.reload();
					$("#write-question-ans-modal").modal('hide');
				}
			},
...
function answersubmit()
{
	...
	if(answer == '')
	{
		$('#errorspan').text('Please write your answers').css('color','red');
	...
	$("#read_answers").modal('hide');

	answers = $.trim(answer);
	itemid = $('#item_id').val();
	userid = $('#loguserid').val();
	parent_id = $('#question_id').val();
	usernames = $('#usernames').val();
	userimges = $('#userimges').val();
	baseurl = getBaseURL();
...
var loguserid = $("#loguserid").val();
if (loguserid == 0) {
	window.location.href = baseurl + "/fantacy-4.0/login";
	return false;
}
if (typeof (atuser) == 'undefined') {
//console.log('Defining atuser array');
...
function show_writeanswer(question_id) {
	$("#write-answer-modal").modal('show');
	$("#question_id").val(question_id);
}
...
var itemurl = btoa(id + "_" + Math.random(1, 9999) + "_" + loguserid);
...
*/
$("#share_img").attr("src", shareImg);
$('.share-img').html('<img src="'+shareImg+'" class="img-responsive" />');
$("#share_item_title").html(title);
$("#share_item_username").html(usernames);
$("#share_item_price").html(username_p);


encry_urlss = encodeURIComponent(urlss);
encry_title = encodeURIComponent(title);
encry_image = encodeURIComponent(shareImg);
var emailbody = encry_urlss;
var emailsubject = "Share Product with '"+encry_title+"'";

//alert(encry_image);
//alert(encry_urlss);
$('#metaurl').attr('content', urlss);
$('#metaimage').attr('content', shareImg);
$('.facebook').attr('href', 'http://www.facebook.com/sharer.php?&display=popup&u=' + encry_urlss + '&t=' + encry_image + '&t=' + encry_title);
...
$('.twitter').attr('href', 'http://twitter.com/share?text=' + title+'&url='+ urlss);
//$('.google').attr('href', 'http://plus.google.com/share?url=' +encry_urlss);
$('.google').attr('href', 'https://mail.google.com/mail/?view=cm&fs=1&tf=1&to=&su='+emailsubject+'&body='+emailbody+'&ui=2&tf=1&pli=1');
$('.linkedin').attr('href', 'http://www.linkedin.com/cws/share?url=' + encry_urlss + '&title=' + encry_title);
$('.stumbleupon').attr('href', 'http://www.stumbleupon.com/submit?url=' + encry_urlss + '&title=' + encry_title);
$('.tumblr').attr('href', 'http://www.tumblr.com/share/link?url=' + encry_urlss + '&name=' + encry_title);
}
...
if (loguserid == 0) {
	...
	$("#share-store-modals").modal('show');
}




var usernames = $('#merchant_name_' + id).html();
var title = $('#store_names_'+id).html();
$("#usernames_popup").text(usernames);

//var itemurl = btoa(id + "_" + Math.random(1, 9999));
var urlss = $('#shop_url'+id).attr('href');
//	var shareImg =  $("#img_"+id).attr('src');
var shareImg = $("#store_img_" + id).attr('src');

//alert(urlss);
$("#share_store_img").attr("src", '');
$("#share_store_img").attr("src", shareImg);
$("#share_title").html(title);
$("#share_username").html(usernames);
...
$('#metaurl').attr('content', urlss);
$('#metaimage').attr('content', shareImg);
//$('.facebook').attr('href', 'http://www.facebook.com/sharer.php?u=' + encry_image + '&t=' + encry_title+'&t='+encry_urlss);
$('.facebook').attr('href', 'http://www.facebook.com/sharer.php?&display=popup&u=' + urlss + '&t=' + encry_image + '&t=' + encry_title);
//$('.twitter').attr('href', 'http://twitter.com/?status=' + encry_urlss);
$('.twitter').attr('href', 'http://twitter.com/share?text=' + title+'&url='+ encry_urlss);
$('.google').attr('href', 'http://plus.google.com/share?url=' + encry_urlss);
$('.linkedin').attr('href', 'http://www.linkedin.com/cws/share?url=' + encry_urlss + '&title=' + encry_title);
$('.stumbleupon').attr('href', 'http://www.stumbleupon.com/submit?url=' + encry_urlss + '&title=' + encry_title);
$('.tumblr').attr('href', 'http://www.tumblr.com/share/link?url=' + encry_urlss + '&name=' + encry_title);
...
if (out[4] == "deal") {
	//alert(out[1]);
	$('.price-strike').html(out[1]);
	$('strike.bold-font').html(out[1]);
...
var urlss = BaseURL + 'listing/' + itemurl;
//var shareImg = $("#img_" + id).html();
var shareImg = $("#img_" + id).text();
//var shareImg = $("#img_" + id).val();
//alert(shareImg);
...
$("#share_item_title").html(title);
$("#share_item_username").html("By "+usernames);
$("#share_item_price").html(username_p);
//alert(urlss);
encry_urlss = encodeURIComponent(urlss);
encry_title = encodeURIComponent(title);
...
function removeusrimg(val) {
	...
	$('#image_computer').val('');
	$('#show_url').attr({
		src: baseurl + 'media/avatars/thumb70/usrimg.jpg'
	});
	$('#removeimg').hide();
}
...
function make_cod_buynoworder() {
	...
	if (shipping_addre == 0) {
		$("#shiperror").show();
		$("#shiperror").html("Please add the shipping address");
		setTimeout(function () {
			$('#shiperr').fadeOut('slow');
		}, 5000);
		return false;
	...
	//alert($('#couponIdb').val()); return false;

	//alert($('#paymentbraintreeform').serialize());
	console.log($('#paymentbraintreeform').serialize());
	//return false;
...
function socialsharef() {
	...
	var meta_image = $("#default_site_logo").val();
	var appID = $("#default_fb_appid").val();
...
//newwindow=window.open('http://www.facebook.com/sharer.php?u='+encry_urlss,'name','height=600,width=600');
newwindow = window.open('https://www.facebook.com/dialog/share?app_id=' + appID + '&display=popup&href=' + encry_urlss, 'name', 'height=600,width=600');
//https://www.facebook.com/dialog/share?app_id=245308646001038&display=popup&href=http%3A%2F%2Ffantacyscript.com%2Fbeta
}
...
{
	let faq_count, offset, limit, item_id, baseurl, cur_count, loguserid;
	faq_count = $('#loadmorevalue').val();
	offset = faq_count;
	limit = 8;
	$('#loadmorevalue').val(parseInt(faq_count)+parseInt(limit));
	item_id = $('#item_id').val();
	cur_count = parseInt(faq_count)+parseInt(limit);
	baseurl=getBaseURL();

	if($('#faq_count').val() < cur_count)
	{
		$(".load_more").hide('slow');
	}else{
		$(".load_more").fadeOut();
	...
	loguserid = $('#loguserid').val();

	$(".load_more").fadeOut('slow');

	$.ajax({
				url: baseurl + 'viewallfaq/'+item_id,
				type: "POST",
				data: {
					'itemid': item_id,
					'loguserid': loguserid,
...
function loadmore(){
		//alert(1);
	let offset, limit, baseurl, loguserid;
	offset = parseInt($('#offsetval').val())+18;
	limit = $('#limitval').val();
	...
	$.ajax({
				...
				success: function (responce) {
					...
					if(responce=='0'){
						...
						$('.allImg').append(responce);
						var firstid = $('.firstid:last').val();
						if(firstid){
							var elmnt = document.getElementById("item_cnt"+firstid);
...
// {
// 	let item_id, user_id, order_id, rating, review;
...
// 	var content = $('#review').html().replace("\n", '<br/>');
...
// 	/*
// 	if( review == '' ){
// 		$('#ratingerrorspan').text('Please leave your review in review box').css('color','red');
// 		//$("#ratingerrorspan").fadeOut(500);
// 		return false;
// 	}else{
// 		$('#ratingerrorspan').text('');
// 	}
// 	*/

// 	if( rating == '' ){
// 		$('#ratingerrorspan').text('Please leave your rating').css('color','red');
// 		return false;
// 	}else{
// 		$('#ratingerrorspan').text('');
// 	}
...
// 				location.reload();				
// 			}else{
// 				$('#ratingerrorspan').html(responce.message);
// 			}
// 		}
// 	});
// }
	...
	*/
	...
	$.ajax({
		...
		success: function ( responce ) {
			...
			if(responce.status == 'true' || responce!="")
			{ 
				console.log("truthy");
				$("#writereview").modal('hide');
				window.location.reload();				
...
function updatereview()
{
	...
	order_id = $('#order_id').val();
	rating = $('#rateval').val();
	review = $('#edit_review_data').val();

	//alert(rating);

	//alert(item_id + review_id + order_id + rating + review);
	/*
	if( review == '' ){
		$('#ratingerrorspan').text('Please leave your review in review box').css('color','red');
		return false;
...
function writereviews(val){
	let itemId = $(val).attr('id');
	$('#item_id').val(itemId);
	$('.current-rate').html($('#avg_rating').val());
	$("#writereview").modal('show');
};

function editreviews(val){
	let itemId = $(val).attr('id');

	let review_data = $('#review_data').val();
	let reviews = review_data.split('~');

	

	var regex = /<br\s*[\/]?>/gi;
	var stringcon = atob(reviews[1]).replace(regex, "\n");
	//alert(stringcon);
	$("#edit_review_data").html(stringcon);

	$('.review_id').val($('#review_id').val());



	//$('#edit_review_data').html(reviews[1]);
	//alert(itemId);
	$('#item_id').val(itemId);
	$('#rateval').val(reviews[0]);
	if($('#avg_rating').val() == '0.5'){
		$('#starhalf').attr('checked','checked');
	}else if($('#avg_rating').val() == '1')
	{
		$('#starhalf').attr('checked','checked');
	}else if($('#avg_rating').val() == '1.5'){
		$('#star1half').attr('checked','checked');
	}else if($('#avg_rating').val() == '2'){
		$('#star2').attr('checked','checked');
	}else if($('#avg_rating').val() == '2.5'){
		$('#star2half').attr('checked','checked');
	}else if($('#avg_rating').val() == '3'){
		$('#star3').attr('checked','checked');
	}else if($('#avg_rating').val() == '3.5'){
		$('#star3half').attr('checked','checked');
	}else if($('#avg_rating').val() == '4'){
		$('#star4').attr('checked','checked');
	}else if($('#avg_rating').val() == '4.5'){
		$('#star4half').attr('checked','checked');
	}else if($('#avg_rating').val() == '5'){
...
//   $("#usProfHeader").css("display", "block");
//   }, function(){
//   $("#usProfHeader").css("display", "none");
// });
